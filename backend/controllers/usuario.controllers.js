//Importar el modelos
const {usuario , alumno , apoderado, tipo_usuario} = require("../models");
//encriptar contraseña
const bcrypt = require("bcrypt");
const jwt = require("jsonwebtoken");
//controlador
const controller = {usuario , alumno , apoderado, tipo_usuario};
...
controller.login = async (req,res) => {
    try{
        ...
        console.log(regisUsuario)
        if(!regisUsuario) return res.status(400).send("Usuario no registrado o equivocado");
        const validPass = await bcrypt.compare(req.body.contraseña, regisUsuario.contraseña);
        if (!validPass){return res.send("Contraseña o usuario incorrecto");} 
        const token = jwt.sign({rut:regisUsuario.rut}, process.env.SECRET_TOKEN);
        return res.header("auth-token",token).send("Esta logueado");
        } catch(error){
...
controller.registroProfesor = async (req,res) => {
    try{
        ...
        if(existRut) return res.status(400).send("Este usuario ya existe");
...
controller.registroAlumno = async (req,res) => {
    try{
        ...
        if(!existRutAlumno){
            ...
            return res.status(400).send("El alumno ya esta registrado");
        }
    }catch(error){
        return res.status(400).send(error);
    }
